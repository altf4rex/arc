...
export const ProjectCard = ({ project } : {project: Project}) => {
  return (
    <div key={project.id} className={`${project?.type || ""} hover-project-block relative flex-wrap`}>
      <Image
        className={`hover-project-img absolute top-0 w-[${project?.formatOfImg?.width}px] h-[${project?.formatOfImg?.height}px]`}
